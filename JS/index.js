...
const getCheckedData = (type) => {      
  const checkedList = document.querySelector(`#${type}Hidden`).value.split(',');  
  const children = document.querySelector(`#${type}Table`).children;
...
const deleteList = (type) => { 
  const index = domList[`${type}Hidden`].value.split(',');   
  const list = [...document.querySelector(`#${type}Table`).children];

  index.forEach(i => {
    return list[i -1].remove();
  })
...
const resetIndex = (type) => {
  document.querySelector(`#${type}Hidden`).value = '';  
}

const resetListIndex = (type) => {
  const list = [...document.querySelector(`#${type}Table`).children]; 
  const listLength = list.length;
  list.forEach((l, index) => {
    l.children[1].textContent = listLength - index;
  })
}
